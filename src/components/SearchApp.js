...
class SearchApp extends React.Component {
  ...
  searchHandler = () =>{
    ...
    axios.post('http://localhost:8080/findById',postData)
    .then(response => {      
  ...
  render() { 
    ...
    Object.keys(sectionList).map((sectionIndex, index) => {
      ...
      if(typeof section.isSearch!='undefined' && section.isSearch){
        ...
        items.push(<FormModel data={section.fields} searchHandler={this.searchHandler} changed = {this.onChangeHandler}/>);
      }
